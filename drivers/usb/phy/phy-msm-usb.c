...
// FIHTDC, IdaChiang, enable debug log for monkey, temp {{
#define DEBUG
#undef pr_debug 
...
// FIHTDC, IdaChiang, for floated charger {{
...
static bool debug_floated_charger_enabled; // FIHTDC, IdaChiang, for floated charger
static bool floated_as_dcp; // FIHTDC, IdaChiang, for floated charger
...
static char fih_otg_disable_mode = 0; // FIHTDC, IdaChiang, add for OTG FREQ
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	bool sm_work_busy; // FIHTDC, IdaChiang, for floated charger 
	u32 cmd_val;
	u32 portsc, config2;
	u32 func_ctrl;
...
lpm_start:
	...
	//floated_charger = motg->chg_type == USB_FLOATED_CHARGER; // FIHTDC, IdaChiang, for floated charger 
...
//	enable_irq(motg->irq); // FIHTDC, IdaChiang, modify for QC 02786627
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	if (motg->chg_type == USB_SDP_CHARGER)
		...
		charger_type = POWER_SUPPLY_TYPE_USB_CDP;
			...
			motg->chg_type == USB_PROPRIETARY_CHARGER) // FIHTDC, IdaChiang, for floated charger 
...
// FIHTDC, IdaChiang, add for OTG FREQ {{
...
static const char *chg_to_string(enum usb_chg_type chg_type)
{
	...
	case USB_UNSUPPORTED_CHARGER:	return "USB_UNSUPPORTED_CHARGER"; // FIHTDC, IdaChiang, for floated charger 
...
// FIHTDC, IdaChiang, for floated charger }}
			...
			motg->chg_type = USB_INVALID_CHARGER;
			msm_otg_notify_charger(motg, 0);
			motg->chg_retry_count = 0; // FIHTDC, IdaChiang, for floated charger
...
static void msm_otg_set_vbus_state(int online)
{
	...
	if (online) {
		...
		motg->is_ext_chg_detected = false; // FIHTDC, IdaChiang, for floated charger
		floated_as_dcp = false; // FIHTDC, IdaChiang, for floated charger
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_REAL_TYPE:
		...
		if (motg->chg_type != USB_INVALID_CHARGER) {
			motg->is_ext_chg_detected = true; // FIHTDC, IdaChiang, for floated charger
			if (motg->chg_type == USB_DCP_CHARGER)
...
// FIHTDC, IdaChiang, add for OTG Enable/Disable {{
