...
#include <linux/alarmtimer.h> // FIHTDC, IdaChiang, add for re-run apsd in slow-insertion issue
...
struct smbchg_chip {
	...
	bool				input_current_setted; // fihtdc, ida chaing, add for FTM
	bool				fih_ftm_mode; // Fihtdc, idachiang, add for FTM
	int				fake_temp; // FIHTDC, IdaChiang, add for FIH REQE
	int				disable_hvdcp_fun; // FIHTDC, IdaChiang, add for disable hvdcp
	int				change_input_current; // FIHTDC, IdaChiang, add for lcm on off feature
	int				show_shutdown_temp; // FIHTDC, IdaChiang, add for shutdown temperature
	int				shutdown_temp; // FIHTDC, IdaChiang, add for shutdown temperature
	int				fih_jeita_full_capacity; // FIHTDC, IdaChiang, add for Nokia UI
	int				fih_comp_set_fun; // FIHTDC, IdaChiang, add for different comp setting
	int				fih_vfloat_comp_high; // FIHTDC, IdaChiang, add for different comp setting
	int				fih_vfloat_comp_low; // FIHTDC, IdaChiang, add for different comp setting
	int				fih_icurrent_comp_high; // FIHTDC, IdaChiang, add for different comp setting
	int				fih_icurrent_comp_low; // FIHTDC, IdaChiang, add for different comp setting
	int				fih_rerun_apsd_enable; // FIHTDC, IdaChiang, add for re-run APSD in slow-insertion issue
	int				fih_rerun_apsd_status; // FIHTDC, IdaChiang, add for re-run APSD in slow-insertion issue
	struct alarm			reapsd_alarm;  // FIHTDC, IdaChiang, add for re-run APSD in slow-insertion issue
	struct work_struct		reapsd_work;  // FIHTDC, IdaChiang, add for re-run APSD in slow-insertion issue
	...
	struct mutex			fih_thermal_lock;// FIHTDC add for thermal control
};
...
// FIHTDC, IdaChiang, add for different comp setting
...
static int smbchg_config_chg_battery_type(struct smbchg_chip *chip)
{
	...
	union power_supply_propval prop = {0,};
	int fastchg_comp_ma=0; // FIHTDC, IdaChiang, add for 4 battery
// FIHTDC, IdaChiang, add for different comp setting {{
	int fih_vcomp_high;
	int fih_vcomp_low;
	int fih_icomp_high;
...
// FIHTDC, IdaChiang, add for 4 battery data {{
	rc = of_property_read_u32(profile_node,
...
static int smbchg_external_otg_regulator_disable(struct regulator_dev *rdev)
{
	...
	if(chip->disable_hvdcp_fun == 1) // FIHTDC, IdaChiang, add for disable HVDCP
		rc = smbchg_sec_masked_write(chip,
				chip->usb_chgpth_base + CHGPTH_CFG,
				HVDCP_EN_BIT, 0);
	else
						...
						enum power_supply_type type)
{
	...
	{
// FIHTDC, IdaChiang, add for thermal issue {{
...
static void smbchg_hvdcp_det_work(struct work_struct *work)
{
	...
	if(chip->disable_hvdcp_fun == 1) // FIHTDC, IdaChiang, add for disable HVDCP, QC patch
	{
		return;
	}
...
static void handle_usb_removal(struct smbchg_chip *chip)
{
	...
	pr_err("usb removal, triggered\n"); // FIHTDC, IdaChiang, add log for monkey test
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_RERUN_AICL:
		chip->input_current_setted =0; // FIHTDC, IdaChiang, add for FTM
		smbchg_rerun_aicl(chip);
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		chip->fake_temp = val->intval; // FIHTDC, IdaChiang, add for FIH Reqe
		break;
	default:
		return -EINVAL;
	}
				       ...
				       enum power_supply_property prop)
{
	...
	case POWER_SUPPLY_PROP_RESTRICTED_CHARGING:
	case POWER_SUPPLY_PROP_ALLOW_HVDCP3:
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX: // FIHTDC, IdaChiang, add for FTM	
	case POWER_SUPPLY_PROP_TEMP: // FIHTDC, IdaChiang, add for FIH Reqe
		rc = 1;
		break;
	default:
		rc = 0;
				       ...
				       union power_supply_propval *val)
{
	struct smbchg_chip *chip = container_of(psy,
				struct smbchg_chip, batt_psy);
	union power_supply_propval psp_ret = {0,}; // FIHTDC, IdaChiang, add for Battery ID
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		...
		break;
	case POWER_SUPPLY_PROP_TEMP:
// FIHTDC, IdaChiang, add for FIH Reqe {{
...
// FIHTDC, IdaChiang, add for FIH Reqe }}
	...
	case POWER_SUPPLY_PROP_MAX_PULSE_ALLOWED:
		...
		break;
// FIHTDC, IdaChiang, add for Battery ID {{		
...
// FIHTDC, IdaChiang, add for BBox log {{
	if (chip->aicl_complete)
	{
		if(aicl_level <= 700)
			printk("BBox::UPD;51::%d::DCP\n", aicl_level); // FIHTDC, IdaChiang, add for BBS log
	}
// FIHTDC, IdaChiang, add for BBox log }}
...
//FTIHTDC, IdaChiang, for 500mA adapter, QC Case 02387520 {{
...
/* disable HVDCP */
// FIHTDC, IdaChiang, add for disable HVDCP, QC patch {{
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	int	fih_comp_fun = 0; // FIHTDC, IdaChiang, add for different comp setting
...
// FIHTDC, IdaChiang add for disable hvdcp {{
...
// FIHTDC, IdaChiang add for disable hvdcp }}
	...
	// FIHTDC, IdaChiang, add for LCM on/off 2A/1A
...
// FIHTDC, IdaChiang, add for re-run APSD in slow-insertion issue }}
	...
	mutex_init(&chip->fih_thermal_lock);// FIHTDC add for thermal control
	...
	chip->fake_temp = 2550;// FIHTDC, IdaChiang, add for FIH REQE
	...
	if(chip->change_input_current == 1)
		rc = device_create_file(chip->dev, &dev_attr_input_change);  // FIHTDC, IdaChiang, add for thermal issue
...
// FIHTDC, IdaChiang, add for ftm mode {{
	chip->fih_ftm_mode = 0;
...
static int smbchg_remove(struct spmi_device *spmi)
{
	...
	if(chip->change_input_current == 1)
		device_remove_file(chip->dev, &dev_attr_input_change); // FIHTDC, IdaChiang, add for thermal issue
