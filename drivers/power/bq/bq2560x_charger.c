...
#endif
#define DEBUG_CHG //FIH,panyu add for dump regs

//#define	JEITA_ALARM_CHECK_NS	5000000000
...
extern bool get_screen_status(void);
...
{
	struct bq2560x *bq = container_of(source, 
							struct bq2560x, bq2560x_ws);
	unsigned long flags;
...
//<FIH,panyu add for FTM test
static int bq2560x_get_chargecurrent(struct bq2560x *bq){
	u8 val = 0;
	int ret;
	...
	if(ret < 0){
		...
		return (((val & REG02_ICHG_MASK) << REG02_ICHG_SHIFT)
			* REG02_ICHG_LSB) + REG02_ICHG_BASE;
	}
...
//<FIH,panyu add for FTM test
int bq2560x_get_input_current_limit(struct bq2560x *bq){
	u8 val = 0;
	int ret;
	...
	if(ret < 0){
		...
		return (((val & REG00_IINLIM_MASK) << REG00_IINLIM_SHIFT)
			* REG00_IINLIM_LSB) + REG00_IINLIM_BASE;
	}
...
static int bq2560x_disable_reset_func(struct bq2560x *bq){
	return bq2560x_update_bits(bq, BQ2560X_REG_07, REG07_BATFET_RST_EN_MASK,
								REG07_BATFET_RST_ENABLE);
}

static int bq2560x_enable_reset_func(struct bq2560x *bq){
	return bq2560x_update_bits(bq, BQ2560X_REG_07, REG07_BATFET_RST_EN_MASK,
...
static int bq2560x_get_prop_charge_status(struct bq2560x *bq)
{
	...
	if (!ret && batt_prop.intval == POWER_SUPPLY_STATUS_FULL){
		bq->report_charge_state = POWER_SUPPLY_STATUS_FULL;
		return POWER_SUPPLY_STATUS_FULL;
	}

	ret = bq2560x_read_byte(bq, &status, BQ2560X_REG_08);
	if (ret) {
		bq->report_charge_state = POWER_SUPPLY_STATUS_UNKNOWN;
	...
	switch(bq->charge_state) {
		case CHARGE_STATE_FASTCHG:
		case CHARGE_STATE_PRECHG:
			bq->report_charge_state = POWER_SUPPLY_STATUS_CHARGING;
			return POWER_SUPPLY_STATUS_CHARGING;
		case CHARGE_STATE_CHGDONE:
			if(bq->batt_warm){
				bq->report_charge_state = POWER_SUPPLY_STATUS_CHARGING;
			//}else if(bq->japan_skuid){//for japan  unsaturated charge
			//	bq->report_charge_state = POWER_SUPPLY_STATUS_FULL;
			}else{
				bq->report_charge_state = POWER_SUPPLY_STATUS_NOT_CHARGING;
			}
			return POWER_SUPPLY_STATUS_NOT_CHARGING;
		case CHARGE_STATE_IDLE:
			bq->report_charge_state = POWER_SUPPLY_STATUS_DISCHARGING;
...
//	POWER_SUPPLY_PROP_CYCLE_COUNT,
	...
	//>
	POWER_SUPPLY_PROP_REAL_STATUS,//FIH,panyu add for soc check
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		val->intval = bq->report_charge_state;		
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		val->intval = bq2560x_get_input_current_limit(bq) * 1000;
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:		
		val->intval = bq2560x_get_chargecurrent(bq) * 1000;
		break;
	//>
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		bq2560x_charging_disable(bq, BATTERY, !val->intval);
		power_supply_changed(&bq->batt_psy);
		power_supply_changed(bq->usb_psy);
		pr_info("POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED: %s\n", 
						val->intval ? "enable" : "disable");
		break;
	//<FIH,panyu add for FTM test
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		bq->ftm_flag=1;
		bq2560x_set_input_current_limit(bq,val->intval / 1000);
		pr_info("POWER_SUPPLY_PROP_INPUT_CURRENT_MAX: %dma\n"
			,val->intval / 1000);
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		bq2560x_set_chargecurrent(bq,val->intval / 1000);
		pr_info("POWER_SUPPLY_PROP_CURRENT_MAX: %dma\n"
			,val->intval / 1000);
		break;
       case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
		bq->therm_lvl_sel = val->intval;
		if(bq->therm_lvl_sel < -1)
			bq->therm_lvl_sel = 0;
		else if(bq->therm_lvl_sel >= bq->thermal_levels)
...
static void bq2560x_check_vbat(struct bq2560x *bq);

#define CDP_CURRENT_MA 1500
static int bq2560x_update_charging_profile(struct bq2560x *bq)
{
	int ret;
	int chg_ma;
	...
	//>end
	thermal_icl_ma = (int)bq->thermal_mitigation[bq->therm_lvl_sel];

	printk("charge volt = %d, charge curr = %d, input curr limit = %d thermal_icl_ma = %d bq->ftm_flag = %d\n",
				chg_mv, chg_ma, icl,thermal_icl_ma,bq->ftm_flag);
	if(icl > thermal_icl_ma)
		icl = thermal_icl_ma;
	if(icl < 100)
		icl = 100;
	if(bq->ftm_flag != 1){
...
static void bq2560x_external_power_changed(struct power_supply *psy)
{
	...
	static bool batt_present = 0;

	//<FIH,panyu+ for link bms psy
	if(!bq->bms_psy){
		bq->bms_psy = power_supply_get_by_name("bms");
	}
	//>
	...
	bq2560x_get_prop_batt_present(bq);
	
	//pr_info("%s()usb_psy_ma = %d,current_limit = %d\n",
	//	__func__,bq->usb_psy_ma, current_limit);
	
	if ((bq->usb_psy_ma != current_limit) ||
		(bq->batt_present != batt_present)) {
		bq->usb_psy_ma = current_limit;
		batt_present = bq->batt_present;
		bq2560x_update_charging_profile(bq);
	}
...
static int bq2560x_regulator_init(struct bq2560x *bq)
{
	...
	regulator_node = of_get_child_by_name(bq->dev->of_node,
				"ti,bq2560x-boost-otg");
    ...
    if(ret) {
        ...
        if (of_find_property(np, "ti,thermal-mitigation",
                ...
                if (bq->thermal_mitigation == NULL) {
                        dev_err(bq->dev, "thermal mitigation kzalloc() failed.\n");
                        return -ENOMEM;
                }
													...
													struct bq2560x * bq)
{
	...
	//add for japan unsaturated charge
/*	if(bq->japan_skuid){
		ret = of_property_read_u32(np,"ti,bq2560x,usb-vreg-jp",&pdata->usb.vreg);
		if(ret) {
			pr_err("Failed to read node of ti,bq2560x,usb-vreg\n");
		}
		
		ret = of_property_read_u32(np,"ti,bq2560x,ta-vreg-jp",&pdata->ta.vreg);
		if(ret) {
...
//	}
	...
	pr_debug("read node of usb-vreg:%d,ta-vreg:%d\n"
		,pdata->usb.vreg,pdata->ta.vreg);
...
static int bq2560x_init_device(struct bq2560x *bq)
{
	...
	if (ret)
                pr_err("Failed to set usbin limt current, ret = %d\n",ret);
...
//<FIH,panyu add for Dynamic condition VDPM
#define DYNAMIC_VDPM_CRITIAL 4000 //MV
#define DYNAMIC_VDPM_MIN 4200 //MV
#define DYNAMIC_VDPM_NORMAL 4400 //MV
#define DYNAMIC_VDPM_MAX 4600 //MV
#define DYNAMIC_VDPM_STEP 200 //MV
#define VBAT_DETECTION_PRECISION 100 //MV
static void bq2560x_check_vbat(struct bq2560x *bq){
	int ret,batt_volt,vdpm_temp;
	static int last_batt_volt = 0;
	union power_supply_propval batt_prop = {0,};
	ret = bq2560x_get_batt_property(bq,
					POWER_SUPPLY_PROP_VOLTAGE_NOW, &batt_prop);
	if (!ret){
		batt_volt = batt_prop.intval / 1000;
		pr_debug("%s get vbat %d\n",__func__,batt_volt);
		if(batt_volt != last_batt_volt){
			last_batt_volt = batt_volt;
			/*
			if(batt_volt > DYNAMIC_VDPM_CRITIAL){			
				bq2560x_set_input_volt_limit(bq,4600);
			}else{
				bq2560x_set_input_volt_limit(bq,4200);
			}*/
			/*
			vdpm_temp = batt_volt + DYNAMIC_VDPM_STEP;
			if(vdpm_temp <= DYNAMIC_VDPM_MIN){
				bq2560x_set_input_volt_limit(bq,DYNAMIC_VDPM_MIN);
			}else if(vdpm_temp >= DYNAMIC_VDPM_MAX){
				bq2560x_set_input_volt_limit(bq,DYNAMIC_VDPM_MAX);
			}else{
				bq2560x_set_input_volt_limit(bq,vdpm_temp);
			}*/
			if(batt_volt <= (DYNAMIC_VDPM_MIN - DYNAMIC_VDPM_STEP)){
				vdpm_temp = DYNAMIC_VDPM_MIN;
			}else if(batt_volt <= (DYNAMIC_VDPM_NORMAL - DYNAMIC_VDPM_STEP)){
				vdpm_temp = DYNAMIC_VDPM_NORMAL;
			}else{
				vdpm_temp = DYNAMIC_VDPM_MAX;
			}
			bq2560x_set_input_volt_limit(bq,vdpm_temp);
		}
	}
...
static void bq2560x_check_jeita(struct bq2560x *bq)
{
	...
	pr_debug("%s,get temp %d\n",__func__,bq->batt_temp);
	...
	pr_debug("cold:%d,cool:%d,warm:%d,hot:%d\n"
		,bq->batt_cold,bq->batt_cool,bq->batt_warm,bq->batt_hot);
	...
	if ((last_cold != bq->batt_cold) || (last_warm != bq->batt_warm) ||
		...
		power_supply_changed(&bq->batt_psy);
		power_supply_changed(bq->usb_psy);
	}else if((abs(last_temp - bq->batt_temp)) >= 1){
		pr_debug("last_temp:%d,batt_temp:%d--change > 1\n",last_temp,bq->batt_temp);
		power_supply_changed(&bq->batt_psy);
	}
							...
							ktime_t now)
{
	...
	//pr_debug("%s:schedule_delayed_work,bq->charge_jeita_work\n",__func__);
...
#else
static void dump_reg(struct bq2560x* bq,u8 addr){
	u8 val = 0;
	int ret;

	ret = bq2560x_read_byte(bq, &val,addr);
	if (ret < 0){
		pr_err("read 0x%x error\n",addr);
	}else{
		pr_err("0x%x : 0x%x\n",addr,val);
	}
}

static void dump_regs(struct bq2560x* bq){
	u8 addr;
	
	for(addr = BQ2560X_REG_00;addr <= BQ2560X_REG_0B;addr++){
		dump_reg(bq,addr);
	}
}
...
//<FIH,panyu add for limit Input current  when Bright screen
//Input limit is 1.5A when Bright screen
#if 0
static int last_screen_status = -1;
#define SCREEN_ON_INPUT_CURRENT_MAX 1500
static int bq2560x_check_screen(struct bq2560x *bq){
	int input_curr;
	int ret;
	bool status = 0;//get_screen_status();

	if(last_screen_status == status){
		return 0;
	}
	
	if(status){
		input_curr = SCREEN_ON_INPUT_CURRENT_MAX;
	}else{
		input_curr = bq->platform_data->ta.ilim;
	}
	ret = bq2560x_set_input_current_limit(bq,input_curr);
	last_screen_status = status;

	pr_debug("screen status:%d,input_curr:%d\n"
		,status,input_curr);
	return ret;
}
#endif
static void bq2560x_charge_jeita_workfunc(struct work_struct *work)
{
	...
	//bq2560x_check_screen(bq);

	bq2560x_dump_status(bq);

	/*schedule_delayed_work(&bq->charge_jeita_work,
			msecs_to_jiffies(calculate_jeita_poll_interval(bq) * 1000));*/
...
static irqreturn_t bq2560x_charger_interrupt(int irq, void *dev_id)
{
	...
	if(!bq->power_good /*&& bq->usb_present*/) {
		if(bq->usb_present != bq->power_good){
			bq->usb_present = bq->power_good;
		...
		//bq2560x_discharge_jeita_workfunc(&bq->discharge_jeita_work.work);		
		//schedule_delayed_work(&bq->discharge_jeita_work,calculate_jeita_poll_interval(bq) * HZ);
		...
		bq2560x_set_watchdog_timer(bq, 40);
...
static int bq2560x_charger_remove(struct i2c_client *client)
{
	...
	ret = bq2560x_write_byte(bq, BQ2560X_REG_07, 0x4C);
...
static void bq2560x_charger_shutdown(struct i2c_client *client)
{
	...
	if (ret) {
		pr_err("Failed to disable batfet, ret=%d\n", ret);
		return;
	}*/
	
	ret = bq2560x_enable_reset_func(bq);
	if (ret) {
		pr_err("Failed to enable_reset_func, ret=%d\n", ret);
		return;
	}
