...
//panyu+ for read skuid
#include <fih/share/cda.h>

#include "bqfs_cmd_type.h"
#include "fih_fg_calibration.h"//for calibration
...
signed int g_battery_id_voltage[TOTAL_BATTERY_NUMBER] = {900000, 1200000};
...
static const struct fg_batt_profile bqfs_image[] = {
	{ bqfs_scud, ARRAY_SIZE(bqfs_scud), 3},
	{ bqfs_mcnair, ARRAY_SIZE(bqfs_mcnair), 3},
};
...
struct bq_fg_chip {
	...
	int batt_tte;
	int	batt_soc;//True SOC
	int batt_report_soc;//FIH,panyu add fot the user space SOC
	//int batt_report_japan_soc;//FIH,panyu add fot the japan SOC
	...
	/*wakeup*/
	struct fg_wakeup_source dumpinfo_wakeup_source;

	/*work*/
	struct delayed_work fih_dump_info_work;	
	struct delayed_work check_temp_capacity_work;
...
//<FIH,panyu add for reset chip
int fg_software_reset(struct bq_fg_chip *bq){
	/*return fg_write_word(bq, bq->regs[BQ_FG_REG_CTRL],
						FG_SUBCMD_SOFT_RESET);*/	
	fg_dm_pre_access(bq);
	mdelay(5);
...
//<FIH,panyu change for batt_status error
static int fg_get_batt_status(struct bq_fg_chip *bq)
{
	if (!bq->batt_present)
		return POWER_SUPPLY_STATUS_UNKNOWN;
		...
		return POWER_SUPPLY_STATUS_DISCHARGING;
//	else if (((bq->japan_skuid) && bq->batt_report_japan_soc == 100) ||
//			(!(bq->japan_skuid) && bq->batt_report_soc == 100))
	else if (bq->batt_report_soc == 100)
		return POWER_SUPPLY_STATUS_FULL;
	else if (bq->batt_curr > 0)
		return POWER_SUPPLY_STATUS_CHARGING;
	else
...
//<FIH,panyu add for modified soc curve
/*The four principles:
1.batt_report_soc Always close to the batt_soc In addition to high temperature and charging
2.batt_report_soc discharge is not increase
3.batt_report_soc Discharge without increasing
4.batt_report_soc Do not jump Out of cold temperature
*/
static int fih_check_temp_capacity(struct bq_fg_chip * bq){
	...
	pr_debug("start:batt_report_soc:%d,batt_soc:%d\n"
		,bq->batt_report_soc,bq->batt_soc);

	if(!bq->batt_soc_ready || bq->batt_soc == -ENODATA){//SOC not ready
		pr_err("real SOC not ready\n");
		return work;
	}else if(bq->batt_report_soc == -EINVAL){//For the first time synchronization
		bq->batt_report_soc = bq->batt_soc;
		pr_debug("Soc first time synchronization\n");
		return work;
	}
	...
	ret = (bq->batt_psy) && 
		!(bq->batt_psy->get_property
			(bq->batt_psy, POWER_SUPPLY_PROP_HEALTH, &val));
	if(!ret){
		pr_err("Read POWER_SUPPLY_PROP_HEALTH error\n");
		return work;
	}
	health = val.intval;
	pr_debug("read POWER_SUPPLY_PROP_HEALTH:%d\n",health);

	//For charging status
	ret = (bq->batt_psy) &&
		!(bq->batt_psy->get_property
			(bq->batt_psy, POWER_SUPPLY_PROP_REAL_STATUS, &val));
	if(!ret){
		pr_err("Read POWER_SUPPLY_PROP_REAL_STATUS error\n");
		return work;
	}
	chg_status = val.intval;
	pr_debug("read POWER_SUPPLY_PROP_REAL_STATUS:%d\n",chg_status);
	
	switch(chg_status){
	case POWER_SUPPLY_STATUS_NOT_CHARGING:
		if(bq->batt_report_soc < bq->batt_soc){
			bq->batt_report_soc++;
			work = 10;
		}else if(POWER_SUPPLY_HEALTH_WARM == health){
			//To solve High temperature charging,Sometimes the SOC will not automatically jump to 100
			if(bq->batt_report_soc < 100){
				bq->batt_report_soc++;
				work = 30;
			}
		}
	...
	case POWER_SUPPLY_STATUS_DISCHARGING:
		//make batt_report_soc approaching batt_soc
		if(bq->batt_report_soc > bq->batt_soc){
	...
	//report_soc  legal detection
	if(bq->batt_report_soc < 0){
		bq->batt_report_soc = 0;
	}else if(bq->batt_report_soc > 100){
		bq->batt_report_soc = 100;
	}
	pr_debug("end:batt_report_soc:%d,batt_soc:%d,work_delay:%d\n"
		,bq->batt_report_soc,bq->batt_soc,work);

	return work;
}
/*Unsaturated charge SOC MAP 
	: (0-JAPAN_FULL_REPORT_SOC) Map to (0-FULL_REPORT_SOC)
*/
/*
#define JAPAN_FULL_REPORT_SOC 	93
#define FULL_REPORT_SOC			100
static void check_japan_capacity(struct bq_fg_chip *bq){
	int res = 0;
	if(!bq->japan_skuid){
		return;
	}

	//Room four and Five into
	res = ((bq->batt_report_soc * FULL_REPORT_SOC) 
				+ (JAPAN_FULL_REPORT_SOC / 2)) 
					/ JAPAN_FULL_REPORT_SOC;
	//report_soc  legal detection
	if(res < 0){
		bq->batt_report_japan_soc = 0;
	}else if(res > 100){
		bq->batt_report_japan_soc = 100;
	}else{
		bq->batt_report_japan_soc = res;
	}

	pr_debug("batt_report_japan_soc:%d,batt_report_soc:%d\n"
		,bq->batt_report_japan_soc,bq->batt_report_soc);
}
*/
/*SOC sleek report*/
#define FIH_CHECK_CAP_MS 1000   //ms
static void fih_check_temp_capacity_work(struct work_struct *work){
	struct bq_fg_chip *bq = container_of(work,
				struct bq_fg_chip,
				check_temp_capacity_work.work);
	int ret;
	
	fg_stay_awake(&bq->dumpinfo_wakeup_source);

	ret = fih_check_temp_capacity(bq);
	if(ret){
		power_supply_changed(&bq->fg_psy);
		schedule_delayed_work(&bq->check_temp_capacity_work,
			msecs_to_jiffies(ret * FIH_CHECK_CAP_MS));
	}
	//check_japan_capacity(bq);
	fg_relax(&bq->dumpinfo_wakeup_source);
}
//>end

//<FIH,panyu add for set taper volage
/*
#define TAPER_VOLAGE_CLASS_ID 	82
#define TAPER_VOLAGE_OFFSET		8
static void fih_set_taper_volage(struct bq_fg_chip * bq,int taper_volage){
	u8 buf[64] = {0};
	int ret;
	static int last_taper_volage = 0;

	pr_debug("last_taper_volage:%d,taper_volage:%d\n",
		last_taper_volage,taper_volage);
	
	if(taper_volage == last_taper_volage){
		return;
	}
	mutex_lock(&bq->update_lock);
	fg_dm_pre_access(bq);
	if(taper_volage){
		ret = fg_dm_read_block(bq,TAPER_VOLAGE_CLASS_ID,
			TAPER_VOLAGE_OFFSET,buf);

		buf[TAPER_VOLAGE_OFFSET - 1] = taper_volage & 0xff;
		buf[TAPER_VOLAGE_OFFSET] = taper_volage >> 8;

		ret = fg_dm_write_block(bq,TAPER_VOLAGE_CLASS_ID,
			TAPER_VOLAGE_OFFSET,buf);
		if(!ret){
			last_taper_volage = taper_volage;
		}
...
static void set_taper_volage_work(struct work_struct *work){
	struct bq_fg_chip *bq = container_of(work,
				struct bq_fg_chip,
				set_taper_volage_work);	
	fih_set_taper_volage(bq,bq->taper_volage);
}
*/
//>end
...
static int fg_psy_register(struct bq_fg_chip *bq)
{
	...
	//<FIH,panyu add
	bq->fg_psy.supplied_to = fg_supplicants;
	bq->fg_psy.num_supplicants = ARRAY_SIZE(fg_supplicants);
	//>
...
static void fg_update_bqfs(struct bq_fg_chip *bq)
{
	...
	pr_err("Fuel Gauge parameter update, reason:%s, version:%d Start...\n", 
			update_reason_str[reason - 1], bqfs_image[bq->batt_id].version);
...
#if 0
//FIH,panyu add
static int fg_pinctrl_init(struct bq_fg_chip *bq){
	int ret;
	struct device_node *np = bq->client->dev.of_node;
	...
	ret = of_property_read_u32(np, "bat_id_gpio_num", &bq->bat_id_gpio_num);
    if(ret) {
		pr_err("Failed to read node of bat_id_gpio_num\n");
		return ret;
	}
	
	bq->fg_pinctrl = devm_pinctrl_get(&bq->client->dev);
	if (IS_ERR_OR_NULL(bq->fg_pinctrl)) {
        pr_err("%s():Target does not use pinctrl\n",__func__);
        ret = PTR_ERR(bq->fg_pinctrl);
        bq->fg_pinctrl = NULL;
        return ret;
    }

	bq->gpio_state = pinctrl_lookup_state(bq->fg_pinctrl,"bat_id_default");
	if (IS_ERR_OR_NULL(bq->gpio_state)) {
        pr_err("%s():Can not get bat_id pinstate\n",__func__);
        ret = PTR_ERR(bq->gpio_state);
        bq->gpio_state = NULL;
        return ret;
    }
...
static int fg_parse_batt_id(struct bq_fg_chip *bq)
{
	...
	ret = pinctrl_select_state(bq->fg_pinctrl, bq->gpio_state);
	if (ret) {
        pr_err("can not set bat_id_default pins\n");
		return -1;
    }else{
		value = gpio_get_value(bq->bat_id_gpio_num);
		if(value < ARRAY_SIZE(bqfs_image)){
			bq->batt_id = value;
		}
		pr_debug("read bat id,gpio%d = %d\n",bq->bat_id_gpio_num,bq->batt_id);
		return 0;
	}
#endif
	if (of_find_property(bq->dev->of_node, "qcom,batid-vadc", NULL)) {
		bq->vadc_dev = qpnp_get_vadc(bq->dev, "batid");
		if (IS_ERR(bq->vadc_dev)) {
			ret = PTR_ERR(bq->vadc_dev);
			pr_err("vadc property missing, ret=%d\n", ret);
			return -1;
		} else {
			ret = qpnp_vadc_read(bq->vadc_dev, P_MUX2_1_1, &adc_result);
			if (ret) {
				pr_err("can not set bat_id_default pins\n");
				return -1;
			}
			id_volt = adc_result.physical;
			bq->batt_id_volt = id_volt;
			bq->connected_rid = 100000*1800/(1800-id_volt/1000)-100000;
		}
	} else {
		pr_err("can not find property batid-vadc");
		return -1;
	}


	 for (i = 0; i < TOTAL_BATTERY_NUMBER; i++) {
                if (id_volt < g_battery_id_voltage[i] + BAT_ID_POS_NEG_VOLTAGE_ERR_RANGE &&
                        id_volt > g_battery_id_voltage[i] - BAT_ID_POS_NEG_VOLTAGE_ERR_RANGE) {
                        bq->batt_id = i;
                        break;
		}
	}
	...
	if(bq->batt_id == -1)
		pr_err("it is invalid battery!!!  batt_id=%d, batt_id_volt=%d, resistance_id=%d\n", bq->batt_id, bq->batt_id_volt, bq->connected_rid);
	else
		pr_err("batt_id=%d, batt_id_volt=%d, resistance_id=%d\n", bq->batt_id, bq->batt_id_volt, bq->connected_rid);
...
//<FIHpanyu add for Asynchronous update battery parameters
#define SOC_POLLING_RETRY_LIMIT 100
static void fg_update_bqfs_workfunc(struct work_struct *work)
{
	...
	if(bq->batt_soc == 0 && bq->batt_volt >= 3450){
		bq->batt_soc = -ENODATA;
		pr_debug("To recalculate the SOC(%d)\n",bq->batt_soc);
		fg_software_reset(bq);
		while(i++ < SOC_POLLING_RETRY_LIMIT) {
			if (bq->batt_soc > 0)
				break;
			msleep(100);
		}
	...
	bq->batt_soc_ready = true;

	power_supply_changed(&bq->fg_psy);
}
//>end

//<FIH,panyu add for  print log
#ifndef DEBUG_FG
static void dump_regs(struct bq_fg_chip *bq){}
#else
static void dump_regs(struct bq_fg_chip *bq){
	int i;
	int ret;
...
#define TEMP_DETA 10
static void fih_check(struct bq_fg_chip *bq)
{
	static int pre_temp = 0, pre_soc = 0;

	if((abs(pre_temp - bq->batt_temp)) >= TEMP_DETA){
		if (bq->batt_psy)
			power_supply_changed(bq->batt_psy);
	}

	if(pre_soc != bq->batt_soc){
		if (bq->batt_psy)
			power_supply_changed(bq->batt_psy);
	}

	pre_temp = bq->batt_temp;
	pre_soc = bq->batt_soc;

}


#define FIH_DUMP_INFO_MS		10000//10s
static void fih_dump_info_workfunc(struct work_struct *work)
{
	struct bq_fg_chip *bq = container_of(work, 
							struct bq_fg_chip, fih_dump_info_work.work);
	int ret;
	...
	fih_check(bq);
	dump_regs(bq);
	schedule_delayed_work(&bq->fih_dump_info_work,
		msecs_to_jiffies(FIH_DUMP_INFO_MS));
	fg_relax(&bq->dumpinfo_wakeup_source);
}

static int fih_dump_info = 1;
static void determine_initial_status(struct bq_fg_chip *bq);
static void delayed_init_work(struct work_struct *work){
	struct bq_fg_chip *bq = container_of(work,
				struct bq_fg_chip,
				init_work);
	fg_update_bqfs_workfunc(&bq->update_work);
	determine_initial_status(bq);
	get_batt_psy(bq);

	if(fih_dump_info){
		fih_dump_info_workfunc(&bq->fih_dump_info_work.work);
	}
}


/*#define CHARGE_ENABLE_CHECK_NS 10000000000 //20s
static enum alarmtimer_restart fih_dump_info_alarm_cb(struct alarm *alarm,
								ktime_t now)
{
	int ret;
	struct bq_fg_chip *bq = container_of(alarm, 
								struct bq_fg_chip, dump_info_alarm);
	
	pm_stay_awake(bq->dev);
	schedule_work(&bq->fih_dump_info_work);

	if(bq->charging){
		ret = alarm_start_relative(&bq->dump_info_alarm,
							ns_to_ktime(CHARGE_ENABLE_CHECK_NS));
		if (ret)
			pr_err("start alarm to retry charge enable failed, ret=%d\n",
						ret);
	}else{
		ret = alarm_try_to_cancel(&bq->dump_info_alarm);
		if (ret)
			pr_err("try_to_cancel alarm failed, ret=%d\n",ret);
...
//>end
/*
//FIH,panyu add for set property to 'battery'
static int set_property_on_chg(struct bq_fg_chip *chip,
		enum power_supply_property prop, int val)
{
	...
	ret.intval = val;
	rc = chip->batt_psy->set_property(chip->batt_psy, prop, &ret);
	if (rc)
...
static irqreturn_t fg_irq_thread(int irq, void *dev_id)
{
	...
	*/
	...
	//schedule_delayed_work(&bq->check_temp_capacity_work,0);
...
static void convert_rid2battid(struct bq_fg_chip *bq)
{
	...
	bq->batt_id = bq->connected_rid > 40 ? 0:1;
}
#endif
							...
							const struct i2c_device_id *id)
{
	...
	u8 *regs;
	//char SKUID[60];//FIH,panyu add for read skuid
	...
	bq->fake_soc 	= -EINVAL;
	bq->fake_temp	= -EINVAL;
	bq->batt_report_soc = -EINVAL;//FIH,panyu add
	//bq->batt_report_japan_soc = -EINVAL;//FIH,panyu add for japan charger

	bq->batt_soc_ready = false;

	//<FIH,panyu add for Japan unsaturated charger
	/*if((fih_cda_get_skuid(SKUID))
		&&(!strcmp(SKUID,"300JP"))){
		bq->japan_skuid = true;
	}else{		
		bq->japan_skuid = false;
	}*/
	//>end
	...
	wakeup_source_init(&bq->dumpinfo_wakeup_source.source,
			"TI_dumpinfo");
	...
	INIT_WORK(&bq->init_work, delayed_init_work);
	INIT_DELAYED_WORK(&bq->fih_dump_info_work, fih_dump_info_workfunc);
	INIT_DELAYED_WORK(&bq->check_temp_capacity_work,fih_check_temp_capacity_work);
	...
	//fg_irq_thread(client->irq, bq);//panyu-
...
err_1:
	...
	cancel_delayed_work_sync(&bq->fih_dump_info_work);
	//cancel_delayed_work_sync(&bq->check_temp_capacity_work);
	cancel_work_sync(&bq->init_work);
	return ret;
}
