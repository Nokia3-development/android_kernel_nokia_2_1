...
enum fg_mem_data_index {
	...
	FG_DATA_CC_CHARGE,
	FG_DATA_VINT_ERR,
	FG_DATA_ACTUAL_CAPACITY_MAH, // FIHTDC, IdaChiang, add for sleep current
...
struct fg_chip {
	...
	int			prev_status;
	int			health;
	int			pre_temperature; // FIHTDC, IdaChiang, add for temperature change
	int			pre_capacity; // FIHTDC, IdaChiang, add for capacity change
	int			fake_fcc; // FIHTDC, IdaChiang, add for FIH Reqe
	int			fake_cc; // FIHTDC, IdaChiang, add for FIH Reqe
	int			fih_ftm_deepsleep; // FIHTDC, IdaChiang, add for sleepcurrent in BB1
	...
	struct delayed_work	fih_thermal_control_work; // FIHTDC add for thermal control
	struct fg_wakeup_source	fih_thermal_wakeup_source; // FIHTDC add for thermal control
};
...
// FIHTDC, IdaChiang, add for temperature changed {{
void fih_temp_check(struct fg_chip *chip)
{
	...
	if(temp_now > chip->pre_temperature)
	{
		if((temp_now - chip->pre_temperature)>=10)
		{
			...
			fih_diff_jeita_comp_set(chip, temp_now); // FIHTDC, IdaChiang, add for different comp setting
...
// FIHTDC, IdaChiang, add for  temperature changed  }}
// FIHTDC, IdaChiang, add for capacity changed {{
...
#define PMIC_CH 17
#define PA0_CH 18
// FIHTDC, IdaChiang, add for dump info {{
int fih_dump_info(struct fg_chip *chip)
{
	...
	// FIHTDC, IdaChiang, add for deepsleep
...
resched:
	fih_temp_check(chip); // FIHTDC, IdaChiang, add for  temperature changed
	fih_dump_info(chip); // FIHTDC, IdaChiang, add for dump info
	schedule_delayed_work(
						...
						enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_INFO:
	case POWER_SUPPLY_PROP_BATTERY_INFO_ID:
	case POWER_SUPPLY_PROP_CHARGE_FULL: // FIHTDC, IdaChiang, add for FIH REQE
	case POWER_SUPPLY_PROP_CYCLE_COUNT:// FIHTDC, IdaChiang, add for FIH REQE
		return 1;
	default:
		break;
	}
...
// FIHTDC, IdaChiang, add for BBoX battery ID {
#ifdef BBS_LOG
...
#ifdef BBS_LOG
	printk("BBox::UPD;49::%d\n", (chip->nom_cap_uah)/1000); // FIHTDC, IdaChiang, add for BBS log
#if !defined(CONFIG_FIH_PROJECT_E2M)
	printk("BBox::UPD;50::%d::%lld\n", fg_get_cycle_count(chip), (chip->learning_data.learned_cc_uah)/1000); // FIHTDC, IdaChiang, add for BBS log
#endif
...
static void fg_cancel_all_works(struct fg_chip *chip)
{
	...
	cancel_delayed_work_sync(&chip->fih_thermal_control_work);// FIHTDC add for thermal control
	alarm_try_to_cancel(&chip->fg_cap_learning_alarm);
	alarm_try_to_cancel(&chip->hard_jeita_alarm);
	if (!chip->ima_error_handling)
...
// FIHTDC, IdaChiang, add for sleep current {{
#ifdef CONFIG_FIH_MT_SLEEP
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->fih_thermal_control_work, fih_thermal_control_work);// FIHTDC add for thermal control
	...
	chip->pre_capacity = 50; // FIHTDC, IdaChiang, add for capacity changed
	chip->pre_temperature = DEFAULT_TEMP_DEGC;// FIHTDC, IdaChiang, add for temperature changed
	chip->fake_fcc = -1; // FIHTDC, IdaChiang, add for FIH Reqe
	chip->fake_cc = -1; // FIHTDC, IdaChiang, add for FIH Reqe
