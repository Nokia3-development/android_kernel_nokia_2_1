...
static unsigned char WeakShort_GetAdcData( int AllAdcDataLen, int *pRevBuffer  )
{
    ...
    for (i = 0; i < 100 * 5; i++) { // Data ready,and FW set reg 0x07 to 0.
        ...
        FTS_TEST_INFO("read 0x07 data=0x%x", (unsigned int)Data);
        if (ReCode == ERROR_CODE_OK) {
            if (Data == 0) {
                bAdcOK = true;
                break;
    ...
    iReadNum = iReadDataLen / pkg_len;
    if (0 != (iReadDataLen % pkg_len)) iReadNum++;

    if (iReadDataLen <= pkg_len) {
        BytesNumInTestMode1 = iReadDataLen;
    } else {
        BytesNumInTestMode1 = pkg_len;
    }

    ReCode = fts_i2c_read_write(pDataSend, 1, pDataSend + 1, BytesNumInTestMode1);
    if (ReCode != ERROR_CODE_OK) {
        FTS_TEST_SAVE_INFO("\nread adc fts_i2c_read_write Failed!1 ");
        goto EndGetAdc;
    }
    ...
    for (i = 1; i < iReadNum; i++) {
        ...
        if (i == iReadNum - 1) { //last packet
            ReCode = fts_i2c_read_write(NULL, 0, pDataSend + 1 + pkg_len * i, iReadDataLen - pkg_len * i);
            if (ReCode != ERROR_CODE_OK) {
                FTS_TEST_SAVE_INFO("\nread adc fts_i2c_read_write Failed!2 ");
                goto EndGetAdc;
            }
        } else {
            ReCode = fts_i2c_read_write(NULL, 0, pDataSend + 1 + pkg_len * i, pkg_len);
            if (ReCode != ERROR_CODE_OK) {
                FTS_TEST_SAVE_INFO("\nread adc fts_i2c_read_write Failed!3 ");
                goto EndGetAdc;
            }
        }
    }

    FTS_TEST_INFO("Read 0xF4 data:");
    for (j = 1; j < iReadDataLen; j++) {
        printk("[%3d]=0x%x,", j, pDataSend[j]);
        if ((j % 10) == 0)
            printk("\n");
    }
    FTS_TEST_INFO("Read 0xF4 data");
