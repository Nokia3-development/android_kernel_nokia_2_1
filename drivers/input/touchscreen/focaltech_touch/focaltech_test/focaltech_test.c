...
*****************************************************************************/
#define SELFTEST_INI_VENDOR1 "FTS_HLT.ini"
#define SELFTEST_INI_VENDOR2 "FTS_TXD.ini"
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    fts_test_save_test_data("touch_data.txt", test_data.store_all_data, data_len);
...
void fts_touch_selftest(void)
{
    ...
    if (ret < 0) {
        FTS_TEST_ERROR("Get vendor id fail");
    }
    if (vendor_id == FTS_VENDOR_ID) {
        FTS_TEST_INFO("Self test for vendor 1");
        sprintf(fwname, "%s", SELFTEST_INI_VENDOR1);
    } else if (vendor_id == FTS_VENDOR_ID2) {
        FTS_TEST_INFO("Self test for vendor 2");
        sprintf(fwname, "%s", SELFTEST_INI_VENDOR2);
    } else {
        FTS_TEST_ERROR("Invalid vendor id, self test for vendor 1");
        sprintf(fwname, "%s", SELFTEST_INI_VENDOR1);
    }
    FTS_TEST_INFO("Self test ini:%s.", fwname);
...
int fts_touch_selftest_result(void)
{
