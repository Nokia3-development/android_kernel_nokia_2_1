...
#define TIMEOUT_READ_REG                    300//1000 //unit:ms
...
static void fts_ts_resume_func(struct work_struct *work);
...
int fts_wait_tp_to_valid(struct i2c_client *client)
{
    ...
    do {
        ...
        if ((ret < 0) || (reg_value != chip_id)) {
            FTS_ERROR("TP Not Ready, ReadData = 0x%x", reg_value);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    INIT_WORK(&ts_data->resume_work, fts_ts_resume_func);
    ...
    touch_cb.touch_selftest = fts_touch_selftest;
    touch_cb.touch_selftest_result = fts_touch_selftest_result;
    touch_cb.touch_tpfwver_read = fts_touch_tpfwver_read;
    touch_cb.touch_tpfwimver_read = NULL;
    touch_cb.touch_fwupgrade = NULL;
    touch_cb.touch_fwupgrade_read = NULL;
    touch_cb.touch_vendor_read = fts_touch_vendor_read;
    touch_cb.touch_scover_write = NULL;
    touch_cb.touch_scover_read = NULL;
#if FTS_GESTURE_EN
    touch_cb.touch_gesture_write = fts_touch_gesture_write;
    touch_cb.touch_gesture_read = fts_touch_gesture_read;
    touch_cb.touch_gesture_available_read = fts_touch_gesture_available_read;
    touch_cb.touch_gesture_available_write = fts_touch_gesture_available_write;
#endif
    touch_cb.touch_alt_rst = NULL;
    touch_cb.touch_alt_st_count = NULL;
    touch_cb.touch_alt_st_enable = NULL;
    tp_probe_success = 1;
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    mutex_lock(&ts_data->resume_mutex);
    ...
    mutex_unlock(&ts_data->resume_mutex);
